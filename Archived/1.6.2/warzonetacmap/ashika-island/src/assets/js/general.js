...
async function main() {
  ...
  addOverlayToMap(control, group_playerSpawn, "Player Spawn", {dmz: true});
  ...
  addOverlayToMap(control, group_chemicalZone, "Radiation Zone", {dmz: true});
  ...
  L.imageOverlay(`${mapPath}/src/assets/images/map/ashika-island-4000.webp`, bounds).addTo(
    map
  );
  ...
  map.on("click", (e) => {
    ...
    if (e.originalEvent.altKey) {
      ...
      showNotificationMessage("Link copied to clipboard!");
      copyToClipboard(
        `${mapPath}?point=${lat.toFixed(3)},${lng.toFixed(3)}`
      );
